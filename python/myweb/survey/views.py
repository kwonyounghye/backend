...
def main(request):
    #filter 조건 검색, -survey_idx 내림차순 정렬
...
def show_result(request):
    ...
    # row() 직접 sql 명령어를 사용하는 방법
    surveyList = Survey.objects.raw('''
    select survey_idx, num, count(num) sum_num, round((select count(*) from survey_answer where survey_idx = a.survey_idx and num = a.num)*100.0/
    (select count(*) from survey_answer where survey_idx = a.survey_idx),1) rate from survey_answer a where survey_idx = %s group by survey_idx, num order by num
    ''', idx)

    surveyList = zip(surveyList, answer) # 같은 인덱스끼리 결합
